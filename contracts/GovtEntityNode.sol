...
contract governmentEntityNode {
    ...
    uint256 public constant len = 30;

    uint256 public rows = len;
    ...
    function checkSurplusPower(uint256 row, uint256 column) public view returns (uint256, uint256)
    {
            if (row > 0) //1
            {
        ...
        for (uint256 i = 0; i < rows; i++) {
            for (uint256 j = 0; j < columns; j++) {
                ...
                if (house.connection()) {
                    ...
                    house.setstate(calculateState(house.getNL(), house.getSoc(), i, j));
                }
            }
        }
    }
